...
const questions = [
  ...
  },
  {
    key: "q6",
    name: "坐著與人交談時",
    sub: "Sitting and talking to someone",
  },
  {
    key: "q7",
    name: "午飯後安靜坐著時（未飲酒）",
...
const severity = computed(() => {
  ...
  if (!isComplete.value)
    return {
      level: "填寫中",
      color: "filling",
      advice: `已完成 ${answeredCount.value}/8 題`,
    };
  if (s <= 10)
    return {
      level: "正常嗜睡",
  ...
  <div class="ess">
    ...
    <div class="ess-group">
      <div class="group-header">
        ...
        <div class="group-score-pill">{{ answeredCount }}/8 題</div>
      </div>
    ...
    <!-- Result card -->
    <div class="ess-result" :class="severity.color">
    ...
    <div v-if="showResults" class="results-detail">
      ...
      <div class="detail-row total-row">
        ...
        <span class="detail-score brand">{{ totalScore }} / 24</span>
    ...
    <p v-if="!isComplete" class="progress-hint">
      已完成 {{ answeredCount }}/8 題，尚餘 {{ 8 - answeredCount }} 題
    </p>
  </div>
</template>
...
/* Score-specific colors — s0 (green) through s3 (amber) */
.opt-pill.s0 {
