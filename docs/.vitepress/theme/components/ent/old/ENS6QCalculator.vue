...
function generateMarkdown() {
  ...
  return (
    ...
    `---\n\n` +
    `## 計算結果\n\n` +
    `- **ENS6Q 總分**：${totalScore.value} / 30\n` +
    `- **嚴重程度**：${severity.value.level}\n` +
    `- **ENS 疑似陽性**：${severity.value.ens ? "是（≥ 11 分）" : "否（< 11 分）"}\n` +
...
<template>
  <div class="ens">
    ...
    <div class="ens-header">
      ...
      <div class="score-badge" :class="severity.color">
        ...
        <span class="score-label">{{
          isComplete ? severity.level : `${answeredCount}/6`
        }}</span>
      </div>
    </div>
    ...
    <div class="severity-bar-wrap">
      ...
      <!-- Ticks: absolutely positioned at true % of 30 -->
      <div class="severity-ticks-abs">
        ...
        <span class="tick-abs" style="left: 60%">18</span>
        <span class="tick-abs" style="left: 80%">24</span>
        <span class="tick-abs" style="left: 100%">30</span>
      </div>
      <div class="severity-labels-abs">
        <span class="label-abs" style="left: 0%">低疑似</span>
        <span class="label-abs tick-cutoff" style="left: 36.67%">陽性門檻</span>
        <span class="label-abs" style="left: 60%">中度</span>
        <span class="label-abs" style="left: 80%">重度</span>
        <span class="label-abs" style="left: 100%">極重</span>
      </div>
    </div>

    <!-- ── ENS positive alert ─────────────────────────────────── -->
    <div v-if="isComplete && severity.ens" class="ens-alert">
      <span class="alert-icon">⚠</span>
      <span>
        <strong>ENS6Q ≥ 11（陽性篩查）：</strong>
        總分
        {{ totalScore }} 分，符合空鼻症疑似診斷標準。建議安排棉花測試（Cotton
    ...
    <div class="intro-bar">
      ...
      <span>
        請依據<strong>目前</strong>您的鼻腔症狀嚴重程度評分。每題 0（無症狀）至
        5（極嚴重），共 6 題，總分 0–30 分。
    ...
    <div class="ens-group">
      <div class="group-header">
        <div class="group-label-block">
          ...
          <span class="group-sub">ENS Core Symptoms · 6 題 · 每題 0–5 分</span>
        </div>
        <div class="group-score-pill">{{ answeredCount }}/6 題</div>
      </div>
      ...
      <div class="item-list">
        ...
        >
          ...
          <!-- Score options — 6-column row (0–5) -->
    ...
    <!-- ── Result card ────────────────────────────────────────── -->
    <div class="ens-result" :class="severity.color">
      ...
      <div class="result-right">
        ...
        <span class="result-calc" v-if="isComplete">
          ENS 疑似陽性門檻 ≥ 11 分 ·
          {{
            severity.ens
              ? `本次 ${totalScore} 分（陽性）`
    ...
    <div v-if="showResults" class="results-detail">
      ...
      <div v-for="q in questions" :key="q.key" class="detail-row">
        ...
        <span
          ...
          :class="{
            zero: selections[q.key] === 0,
            high: selections[q.key] >= 4,
            brand: selections[q.key] > 0 && selections[q.key] < 4,
          }"
        >
          {{ selections[q.key] >= 0 ? selections[q.key] : "—" }}
        </span>
      ...
      <div class="detail-row total-row">
        ...
        <span class="detail-score brand">{{ totalScore }} / 30</span>
    ...
    <p v-if="!isComplete" class="progress-hint">
      已完成 {{ answeredCount }}/6 題，尚餘 {{ 6 - answeredCount }} 題
    </p>
  </div>
</template>
