...
| 方法       | 起始劑量                             | 調整方式                                   | 頻率    |
| ---------- | ------------------------------------ | ------------------------------------------ | ------- |
| **方法一** | 10 units/day 或 0.1–0.2 units/kg/day | FBG > 130 mg/dL → 增加 10–15% 或 2–4 units | 每 3 天 |
| **方法二** | 同方法一                             | FBG > 130 mg/dL → 每天增加 1 unit          | 每天    |
...
| 口服藥             | 建議                                                                |
| ------------------ | ------------------------------------------------------------------- |
| **Metformin**      | 繼續合併使用，除非有禁忌                                            |
| **Pioglitazone**   | 考量胰島素阻抗時可使用；注意體液滯留與體重增加風險                  |
| **SUs / Glinides** | Basal-Plus 治療時可續用（減少 BI 劑量），但低血糖及體重增加風險較高 |
| **DPP-4i**         | 可繼續使用                                                          |
...
- 發生低血糖（< 70 mg/dL）
- **基礎劑量 > 0.5 units/kg/day** 但 FBG 已達標、A1C 仍未達標 → 應評估加入 **餐時胰島素（Bolus）** 或 GLP-1 RA，不應繼續增加基礎劑量

### ⚠ Toujeo（Glargine U-300）注意事項
...
| --------------- | ---------------------------- | ------------------------- |
| 濃度            | 100 U/mL                     | 300 U/mL（3 倍）          |
| 1 unit 對應體積 | 0.01 mL                      | 0.00333 mL（1/3）         |
...
- **從其他基礎胰島素轉換**：建議個別化調整，通常以原劑量的 80% 起始後依 FBG 微調
...
1. OADs+BI 調整後 FBG 已達 100–110 mg/dL，但 A1C 仍不能達 < 7.0%
...
| 藥物               | 速效成分   | 長效成分             | 注射時機                  |
| ------------------ | ---------- | -------------------- | ------------------------- |
| **NovoMix 30**     | 30% Aspart | 70% Protamine Aspart | 餐前立即注射              |
| **Humalog Mix 25** | 25% Lispro | 75% Lispro Protamine | 餐前 15 分鐘              |
| **Humalog Mix 50** | 50% Lispro | 50% Lispro Protamine | 餐前立即注射（高 CHO 餐） |
| **Ryzodeg**        | 30% Aspart | 70% Degludec         | BID 早晚餐前各一次隨主餐  |

- **起始劑量**：以基礎胰島素劑量分成 2/3 早餐前、1/3 晚餐前，或早晚餐前各一半；或 0.2–0.4 units/kg/day
...
| 參數                      | 速效胰島素               | 短效 Regular |
| ------------------------- | ------------------------ | ------------ |
| **ISF（胰島素敏感係數）** | 1800 ÷ TDD               | 1500 ÷ TDD   |
| **碳水化合物比值**        | 500 ÷ TDD                | 450 ÷ TDD    |
| **TDD 估算**              | 0.5 units/kg（初次使用） | —            |
...
- 餐後 2h 血糖目標：< 180 mg/dL
- 每次僅依一種方法調整，避免疊加
- 老年人、腎功能不全或低血糖高風險族群，起始劑量應減量 20–50%

> 本工具依據 2022 ADA 治療指引及臺灣糖尿病學會建議製作，僅供臨床參考。實際劑量調整請依個別病人狀況、共病、低血糖病史及醫囑決定。
